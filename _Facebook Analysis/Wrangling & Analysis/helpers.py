...
def bar_peaks(df_head, title, threshold=0, xlabel='Comments Creation Date'):
    """
...
def text_peaks(df, mytext, n=5, threshold=0, mode='equal'):
    """
    A function to filter on specific text contributions and plot bar graph for peaks
    Args:
        - df --> the dataframe to be used.
        - mytext (str) --> the text to filter on.
        - mode --> 'equal' to filter on the exact text 
        or 'contain' to filter on any comments containing the text
        or 'list' to filter on a list of keywords
