...
def contr_peaks(df, user, n=5, threshold=0):
    ...
    df_user_daily = df_user.groupby('date')['translated'].count().to_frame().reset_index().rename(columns={'translated': 'ntweets'})
...
def keyword_peaks(df, keyword, n=5, threshold=0):
    ...
    """
    df_keyword = df[df.translated.str.contains(keyword)]
    df_keyword_daily = df_keyword.groupby('date')['translated'].count().to_frame().reset_index().rename(columns={'translated': 'ntweets'})
...
def text_peaks(df, mytext, n=5, threshold=0):
    ...
    """
    df_text = df[df.translated == mytext]
    df_text_daily = df_text.groupby('date')['translated'].count().to_frame().reset_index().rename(columns={'translated': 'ntweets'})
...
def text_all_peaks(df, mytext, n=5, threshold=0):
    ...
    Args:
        ...
        - mytext (str) --> the text to filter on.
    """
    df_text = df[df.translated.str.contains(mytext)]
    df_text_daily = df_text.groupby('date')['translated'].count().to_frame().reset_index().rename(columns={'translated': 'ntweets'})
    df_text_daily.sort_values(['ntweets', 'date'], ascending=[False, False], inplace=True)
